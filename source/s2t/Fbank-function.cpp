...
namespace s2t {
    ...
    SplitRadixComplexFft<Real>::SplitRadixComplexFft(
        ...
        memcpy(brseed_, other.brseed_, sizeof(INT32) * brseed_size);
        ...
        if (logn_ < 4) {
            ...
            for (INT32 i = logn_; i >= 4; i--) {
                ...
                memcpy(tab_[i - 4], other.tab_[i - 4],
    ...
    void ComplexFftRecursive(Real* data, int nffts, int N,
          ...
          // this block of code could be removed and it would still work.
            ...
            if (nffts > 1 && size_perblock * nffts > COMPLEXFFT_BLOCKSIZE) {  // can break it up...
                ...
                if (block_skip < nffts) {
                    ...
                    while (blocks_left > 0) {
                        int skip_now = min(blocks_left, block_skip);
        ...
        */
        ...
        if (P > 1 && Q > 1) {  // Do the rearrangement.   C.f. eq. (8) below.  Transform
            ...
            int dimSize = { N };
            if (tmp_vec->order == -1) tmp_vec->Resize(1, &dimSize);
            if (tmp_vec->GetDim(0) < (INT32)N) tmp_vec->Resize(1, &dimSize);
            int index = { 0 };
            Real* data_tmp = reinterpret_cast<Real*>(tmp_vec->GetCell(&index, 1));
        ...
        {  // Do the multiplication
          ...
          // inside.
            int dimSize = { P * 2 };
            if (tmp_vec->GetDim(0) < (INT32)(P * 2)) tmp_vec->Resize(1, &dimSize);
            int index = { 0 };
            Real* temp_a = reinterpret_cast<Real*>(tmp_vec->GetCell(&index, 1));
                    ...
                    for (int pd = 0; pd < P; pd++) {  // pd == p'
                        ...
                        if (P > 2) {
                            ...
                            for (int p = 2; p < P; p++) {
                                ...
                                int data_idx = p * Q + qd;
                                ComplexAddProduct(p_pdQ_qd_re, p_pdQ_qd_im, data_thisblock[data_idx * 2], data_thisblock[data_idx * 2 + 1], &(temp_a[pd * 2]), &(temp_a[pd * 2 + 1]));
                            }
                        }
                        if (pd != P - 1)
    ...
    template<typename Real> void ComplexFft(XTensor* v, bool forward, XTensor* tmp_in) {
        ...
        XTensor tmp;  // allocated in ComplexFftRecursive.
        int index = { 0 };
        tmp.Init();
        ComplexFftRecursive<Real>(reinterpret_cast<Real*>(v->GetCell(&index, 1)), 1, N, factor_beg, factor_beg + factors.size(), forward, (tmp_in ? tmp_in : &tmp));
    }
    template void ComplexFft<float>(XTensor* v, bool forward, XTensor* tmp_in);
    template void ComplexFft<double>(XTensor* v, bool forward, XTensor* tmp_in);
    ...
    template<typename Real> void RealFft(XTensor* v, bool forward) {
        ...
        if (forward) ComplexFft<Real>(v, true);
        int index = { 0 };
