...
namespace s2t {
    ...
    struct FrameExtractionOptions {
        ...
        { 
	        strcpy(windowType, "hanning_periodic");
            strcpy(padMod, "reflect");
	}
...
*/
    ...
    */
    ...
    public:
        ...
        */
        ...
        void Read() {
            ...
            std::cout << "Audio:" << computer_.GetFrameOptions().inputAudio << std::endl;
            if (!inFile) {
    ...
    void OfflineFeatureTpl<F>::Compute(
        ...
        INT32 in_loop_step = 50;
        INT32 out_loop_step = rows_out / in_loop_step;
        INT32 final_loop_step = rows_out % in_loop_step;
        for (INT32 r = 0; r < out_loop_step; r++) {
                futures.push_back(pool.submit([&, r, in_loop_step](FbankComputer& computer_,FeatureWindowFunction& feature_window_function_) {
                    XTensor window;  // windowed waveform.
                    for (INT32 r_in = 0; r_in < in_loop_step; r_in++) {
                        float raw_log_energy = 0.0;
                        ExtractWindow(0, wave, r* in_loop_step +r_in, computer_.GetFrameOptions(),
                        ...
                        output->SetData(output_row.GetCell(startIndex, 2), cols_out, (r * in_loop_step +r_in) * cols_out);
                    }
                }, computer_, feature_window_function_));
        }
        for (std::future<void>& future : futures) {
             future.wait();
        }

        for (INT32 r = 0; r < final_loop_step; r++) {
            float raw_log_energy = 0.0;
            XTensor window;  // windowed waveform.
            ExtractWindow(0, wave, (in_loop_step * out_loop_step + r), computer_.GetFrameOptions(),
                feature_window_function_, window, (use_raw_log_energy ? &raw_log_energy : NULL));
            XTensor output_row;
            int rowDimSize = { rows_out };
            output_row.Resize(1, &rowDimSize);
            computer_.Compute(raw_log_energy, vtln_warp, &window, output_row);
            output->SetData(output_row.GetCell(startIndex, 2), cols_out, (in_loop_step * out_loop_step + r) * cols_out);
        }
